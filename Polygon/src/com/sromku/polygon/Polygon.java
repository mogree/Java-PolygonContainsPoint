...
public class Polygon {
	...
	private Polygon(List<Line> sides, BoundingBox boundingBox, int id) {
		_sides = sides;
	...
	public static class Builder {
		...
		public Builder addVertex(Point point) {
			...
			if (size > 1) {
				Line Line = new Line(_vertexes.get(size - 2), point,
						_sides.size() > 0 && size == 2 ? true : false);
				_sides.add(Line);
			}
		...
		public Builder close() {
			...
			// add last Line
			_sides.add(new Line(_vertexes.get(_vertexes.size() - 1), _vertexes
					.get(0)));
			_isClosed = true;
		...
		public Polygon build() {
			...
			if (!_isClosed) {
				// add last Line
				_sides.add(new Line(_vertexes.get(_vertexes.size() - 1),
						_vertexes.get(0)));
			}

			Polygon polygon = new Polygon(_sides, _boundingBox, _id);
			return polygon;
		}
	...
	private boolean intersect(Line ray, Line side) {
		...
		if (!ray.isVertical() && !side.isVertical()) {
			...
			float x = ((side.getB() - ray.getB()) / (ray.getA() - side.getA())); // x
																					// =
																					// (b2-b1)/(a1-a2)
	...
	private Line createRay(Point point) {
		...
		Point outsidePoint = new Point(_boundingBox.xMin - epsilon,
				_boundingBox.yMin);
